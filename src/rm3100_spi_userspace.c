...
int get_measurement_ready_drdy()
{
	int err = gpiod_line_get_value(line);
	if (err == -1)
		return end_drdy();
	return err;
}
...
struct Measurements get_measurement(int timeout, bool use_drdy)
{
	...
	// special bit manipulation since there is no 24 bit signed int data type
